...
bool LoadFromObjectFile(std::string fileName, Vertex** vertsModel, unsigned short** indsModel, int* vertsModelCnt, int* indsModelCnt)
{
    ...
    while (!f.eof())
    {
        ...
        if (line[0] == 'v')
        {
            ...
            int c1, c2, c3;
            do {
                c1 = rand() % 2; c2 = rand() % 2; c3 = rand() % 2;
            } while (c1 + c2 + c3 >= 2.4 || c1 + c2 + c3 <= 0.8);

            v.r = c1; v.g = c2; v.b = c3;
            verts.push_back(v);
        }
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    if (!LoadFromObjectFile("Caça.obj", &vertices, &indices, &vertsCnt, &indsCnt)) {
		MessageBoxA(0, "Erro ao carregar o modelo, coloque o modelo .obj no mesmo diretorio do .exe", "Erro", 0);
        return -1;
    }
    ...
    while (msg.message != WM_QUIT)
    {
        if (PeekMessage(&msg, 0, 0, 0, PM_REMOVE)) { TranslateMessage(&msg); DispatchMessage(&msg); }
            ...
            XMMATRIX model = XMMatrixScaling(0.2f, 0.2f, 0.2f) * XMMatrixRotationY(t / 2.0f) * XMMatrixRotationX(t / 3.0f);
            XMMATRIX view = XMMatrixLookAtLH({ 0, 1, -20 }, { 0, 0, 0 }, { 0, 1, 0 });
